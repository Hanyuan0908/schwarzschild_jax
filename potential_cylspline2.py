...
# ])
# # Example: interpolate HYPERGEOM_0 using JAX's RegularGridInterpolator
# # axis0: m (0..12), axis1: n (0..8)
# m_grid = jnp.arange(HYPERGEOM_0.shape[0])
# n_grid = jnp.arange(HYPERGEOM_0.shape[1])
# HYPERGEOM_0_interp = jax.scipy.interpolate.RegularGridInterpolator((m_grid, n_grid), 
#                                                                    HYPERGEOM_0, method="linear",
#                                                                      bounds_error=False, fill_value=None)

# HYPERGEOM_I = jnp.array([
...
# m_grid = jnp.arange(HYPERGEOM_0.shape[0])
# n_grid = jnp.arange(HYPERGEOM_0.shape[1])
# HYPERGEOM_I_interp = jax.scipy.interpolate.RegularGridInterpolator((m_grid, n_grid), 
#                                                                    HYPERGEOM_I, method="linear",
#                                                                      bounds_error=False, fill_value=None)

# HYPERGEOM_1 = jnp.array([
...
# m_grid = jnp.arange(HYPERGEOM_0.shape[0])
# n_grid = jnp.arange(HYPERGEOM_0.shape[1])
# HYPERGEOM_1_interp = jax.scipy.interpolate.RegularGridInterpolator((m_grid, n_grid), 
#                                                                    HYPERGEOM_1, method="linear",
#                                                                      bounds_error=False, fill_value=None)

# Q_PREFACTOR = jnp.array([
...
# ])
# Q_PREFACTOR_interp = jax.scipy.interpolate.RegularGridInterpolator(
#     [m_grid], Q_PREFACTOR, method="linear", bounds_error=False, fill_value=None
# )

# X_THRESHOLD1 = jnp.array([0.94,0.956,0.968,0.98,0.98,0.983,0.987,0.989,0.99,0.992,0.992,0.9928,0.993])
# X_THRESHOLD1_interp = jax.scipy.interpolate.RegularGridInterpolator(
#     [m_grid], X_THRESHOLD1, method="linear", bounds_error=False, fill_value=None
# )
# X_THRESHOLD0 = jnp.array([0.72,0.72,0.80,0.80,0.83,0.86,0.85,0.88,0.88,0.88,0.885,0.90,0.91])
# X_THRESHOLD0_interp = jax.scipy.interpolate.RegularGridInterpolator(
#     [m_grid], X_THRESHOLD0, method="linear", bounds_error=False, fill_value=None
# )
...
# def _hypergeom_m(m: int, x: float):
    
#     if x < X_THRESHOLD1_interp(jnp.array([m]))[0]:
#         interp = HYPERGEOM_0_interp if x < X_THRESHOLD0_interp(jnp.array([m]))[0] else HYPERGEOM_I_interp
#         xA8 = x + interp(jnp.array([m, 8]))[0]
#         xA6 = x + interp(jnp.array([m, 6]))[0] + interp(jnp.array([m, 7]))[0] / xA8
#         xA4 = x + interp(jnp.array([m, 4]))[0] + interp(jnp.array([m, 5]))[0] / xA6
#         xA2 = x + interp(jnp.array([m, 2]))[0] + interp(jnp.array([m, 3]))[0] / xA4
#         F = interp(jnp.array([m, 0]))[0] + interp(jnp.array([m, 1]))[0] / xA2

#         return F
#     else:
#         interp = HYPERGEOM_1_interp
#         y = 1.0 - x
#         y2 = y*y
#         z = np.log(max(y, jnp.finfo(float).tiny))
#         F = (interp(jnp.array([m, 0]))[0] + interp(jnp.array([m, 1]))[0] * z +
#              (interp(jnp.array([m, 2]))[0] + interp(jnp.array([m, 3]))[0] * z) * y +
#              (interp(jnp.array([m, 4]))[0] + interp(jnp.array([m, 5]))[0] * z + (interp(jnp.array([m, 6]))[0] + interp(jnp.array([m, 7]))[0] * z) * y + (interp(jnp.array([m, 8]))[0] + interp(jnp.array([m, 9]))[0] * z) * y2) * y2)
...
#         print(xv)
#         if is_halfint:
#             pref = Q_PREFACTOR_interp(jnp.array([m]))[0] / np.sqrt(xv) / (xv**m)
#             # print(pref)
