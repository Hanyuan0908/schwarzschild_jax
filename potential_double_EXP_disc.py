...
def mn_phi_cyl(R, z, M=1e9, a=6.5, b=0.26, G=4.3e-6):
    """
...
def mn_rho_cyl(R, z, M=1e9, a=6.5, b=0.26):
    """
...
def mn_phi_cart(x, y, z, M=1e9, a=6.5, b=0.26, G=4.3e-6):
    x = np.asarray(x, float); y = np.asarray(y, float); z = np.asarray(z, float)
    R = np.sqrt(x*x + y*y)
    return mn_phi_cyl(R, z, M=M, a=a, b=b, G=G)

def mn_rho_cart(x, y, z, M=1e9, a=6.5, b=0.26):
    x = np.asarray(x, float); y = np.asarray(y, float); z = np.asarray(z, float)
    R = np.sqrt(x*x + y*y)
...
if __name__ == "__main__":
    ...
    cs = CylSpline(rho_gt, NR = 30, Nz_pos = 30, mmax = 7, Rmin = 1e-2, Rmax = 20.0, Zmin = 1e-2, Zmax = 15.0,)
    cs.compute_rho_m()
    # 3a) compute Φ_m with fixed tensor grid (parallel/JAX-friendly)
    # cs.compute_phi_m_grid_fixed(N_int=25000, rule="simpson")
    ...
    plt.tight_layout()
    fig.savefig("//Users/hanyuan/Desktop/PhD_projects/Schwarchild_bar/cylspline_surface_density_maps.png", dpi=150)
    # plt.show()
    ...
    for i in tqdm(range(len(Z))):
        ...
        Sigma_fit_faceon += (pot_agama.density(points) * dZ).reshape(X.shape)

    # point = np.column_stack((X.ravel(), Y.ravel()))
    # Sigma_fit_faceon = pot_agama.projectedDensity(point, beta = 0).reshape(X.shape)
    ...
    for i in tqdm(range(len(X2))):
        ...
        Sigma_fit_sideon += (pot_agama.density(points) * dX).reshape(Yg.shape)
    ...
    plt.tight_layout()
    # fig.savefig("//Users/hanyuan/Desktop/PhD_projects/Schwarchild_bar/multipole_surface_density_maps_agama.png", dpi=150)
    # plt.show()
    ...
    plt.tight_layout()
    fig.savefig("//Users/hanyuan/Desktop/PhD_projects/Schwarchild_bar/cylspline_projected_potentials_maps.png", dpi=150)
    plt.show()
